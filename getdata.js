    ...
    $(document).ajaxError(
        function ( /** Event */ event, /** XMLHttpRequest */ jqXHR, /** Object */ ajaxSettings) {
            if (jqXHR.readyState === 4) {
                ...
                $failure.append($('<span/>').text(('AJAX call to ' + ajaxSettings.url + ' failed: ' + jqXHR.status)
                        .trim()))
                    .append($('<br/>'));
...
class Eta {
    constructor( /** String */ route_id, /** Boolean */ direction, /** Date */ time, /** String */ destination, /** String */
        remark) {
        this.route_id = route_id;
        this.direction = direction;
        this.time = time;
        ...
        $.getJSON(
            ...
            function ( /** Object */ json) {
                if (json.data.hasOwnProperty('stop')) {
                    stops[stop_id] = new Stop(json.data.stop, json.data.name_tc);
                }
                --get_stop.remaining;
            }
        );
    ...
    if (!route_stop.hasOwnProperty(route_id)) {
        ...
        [false, true].forEach(
            function ( /** Boolean */ direction) {
                $.getJSON(
                    base_url + 'v1/transport/citybus-nwfb/route-stop/' + routes[route_id].company + '/' +
                    routes[route_id].id + '/' +
...
function get_route_list( /** String */ company_id) {
    $.getJSON(
        ...
        function ( /** Object */ data) {
            data.data.forEach(
                function ( /** Object */ json) {
                    let route = new Route(json.co, json.route, json.orig_tc, json.dest_tc);
            ...
            if (batch === get_all_etas.batch) {
                data.data.forEach(
                    function ( /** Object */ json) {
                        etas.push(
                            new Eta(
                                json.route, json.dir === 'I', json.eta === "" ? null : new Date(json.eta),
                                json.dest_tc, json.rmk_tc
                            )
                        );
                    }
                );
...
$(document).ready(
    function () {
        ...
        function get_all_etas_from_ui() {
            get_all_etas(
                $('#stop_list option:selected').first().val(), $('#common_route_list option:selected').map(
        ...
        function load_route( /** Route */ route, /** Boolean */ direction) {
            ...
            $('#direction_text').text((direction ? route.destination : route.origin) + ' → ' + (direction ? route.origin :
                route.destination));
            $stop_list.empty().append($('<option/>')).append(
                route_stop[route.id][+direction].map(
                    function ( /** RouteStop */ route_stop) {
                        ...
                        return $('<option></option>').attr('value', stop_id).text(stop_id + ' ' + (stop ===
                            null ? '' : stop.name));
                    }
                )
            ).removeAttr('disabled');
                        ...
                        stop_route[stop_id].map(
                            function ( /** RouteStop */ data) {
                                ...
                                const $element = $('<option></option>')
                                    .attr('value', inner_route.company + '-' + inner_route_id + '-' + (
                                        inner_direction ? 'I' : 'O'))
                                    .text(inner_route_id + ' ' + (inner_direction ? inner_route.destination :
                                        inner_route.origin) + ' → ' + (inner_direction ? inner_route.origin :
                                        inner_route.destination));
                                if (inner_route_id === $route.val() && inner_direction === ($direction.val() ===
                                        'I')) {
